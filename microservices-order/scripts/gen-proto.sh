...
if ! command -v protoc-gen-go-grpc &> /dev/null; then
    echo "错误: protoc-gen-go-grpc 未安装。请运行: go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@v1.3.0"
    exit 1
fi
...
echo "proto 代码生成完成！"
echo "生成的文件："
echo "  - order/order.pb.go (消息定义)"
echo "  - order/order_grpc.pb.go (gRPC 服务定义)"
