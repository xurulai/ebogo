...
func InitSrvClient() error {
	...
	orderConn, err := grpc.Dial(
		fmt.Sprintf("consul://%s/%s?wait=14s", config.Conf.ConsulConfig.Addr, config.Conf.OrderService.Name),
		grpc.WithDefaultServiceConfig(`{"loadBalancingPolicy": "round_robin"}`),
		grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithBlock(),
		grpc.WithTimeout(5*time.Second),
	...
	if err != nil {
		fmt.Printf("Failed to dial order_srv: ConsulAddr=%s, ServiceName=%s, err=%v\n",
			config.Conf.ConsulConfig.Addr, config.Conf.OrderService.Name, err)
		return err
	}
	OrderCli = proto.NewOrderClient(orderConn)
