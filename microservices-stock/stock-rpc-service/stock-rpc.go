...
import (
	...
	"stock-rpc-service/internal/server"
...
// 启动参数：-f 指定配置文件路径
var configFile = flag.String("f", "etc/stock-rpc.yaml", "the config file")

// 程序入口：
// 1) 加载配置并初始化依赖
// 2) 创建并启动 gRPC 服务，注册 StockService 服务实现
...
func main() {
	...
	s := zrpc.MustNewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		stockpb.RegisterStockServiceServer(grpcServer, server.NewStockServer(ctx))
